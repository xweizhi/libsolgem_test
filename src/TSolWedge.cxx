...
TSolWedge::Bounds() const
{
  ...
  v[2] = x[0];
  v[3] = y[0];
  //cout<<"bound "<<x[0]<<" "<<y[0]<<endl;
  //cout<<"bound "<<x[1]<<" "<<y[1]<<endl;
  //cout<<"bound "<<x[2]<<" "<<y[2]<<endl;
  //cout<<"bound "<<x[3]<<" "<<y[3]<<endl;
...
//cout<<"bound: "<<v[0]<<" "<<v[1]<<" "<<v[2]<<" "<<v[3]<<endl;
...
void TSolWedge::SetGeometry(TGEMShape* theShape)
{
  double r0 = dynamic_cast<TSolWedge*>(theShape)->GetR0();
  double r1 = dynamic_cast<TSolWedge*>(theShape)->GetR1();
  double phi0 = dynamic_cast<TSolWedge*>(theShape)->GetPhi0();
  double dphi = dynamic_cast<TSolWedge*>(theShape)->GetDPhi();
  SetGeometry(r0, r1, phi0, dphi);
}
//_________________________________________________________________
void
                        ...
                        const Double_t dphi)
{
  ...
  //cout<<"before rotation to lab: "<<xc<<" "<<yc<<endl;
  WedgeToLab (xc, yc);
  //cout<<"phi after rotation to lab: "<<atan2(yc, xc)*180./3.1415926<<endl;
  fOrigin[0] = xc;
  fOrigin[1] = yc;
...
void TSolWedge::Print()
{
  cout << "  Wedge geometry: r0: " << GetR0()
       << " r1: " << GetR1()
       << " phi0: " << GetPhi0()*TMath::RadToDeg()
