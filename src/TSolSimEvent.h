...
public:
  ...
  Int_t GetNECHits() const { return fECHitData.size(); }
  ...
  std::vector<DigiGEMStrip> fGEMStrips; // Digitized strip amplitudes of the GEMs
	...
	struct ECHitData {
	Int_t       fPID;        //PID of the hit
	Int_t       fTID;        //TID of the hit
	Short_t     fPlane;      //0 for large angle EC, 1 for forward angle EC
	Float_t     fEdep;       //energy deposition of the particle hitting EC (only good for electron and photon)
	Float_t     fXPos;       //x position of the hit
	Float_t     fYPos;       //y position of the hit
	Float_t     fZPos;       //z position of the hit    
	};

	std::vector<ECHitData> fECHitData;
	
  ClassDef(TSolSimEvent, 6) // Simulated data for one event
};
