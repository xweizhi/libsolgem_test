...
Int_t TSolEVIOFile::ReadNextEvent(){
	...
	fprintf(stderr, "%s %s line %d: Processing digitized (integer data) hits\n",
		__FILE__, __FUNCTION__, __LINE__ );
#endif//DEBUG
        std::cout<<(*iter)->isContainer()<<std::endl;
	if( (*iter)->isContainer() ){
...
/////////////////////////////////////////////////////////
ECdata::ECdata(int detid):hitdata(detid, __ECDATA_SIZE){
}

int ECdata::GetPlane() const
{
	if (fDetID == __FAEC_VIRTUAL_ID){
		return 1;
	}else if (fDetID == __LAEC_VIRTUAL_ID){
		return 0;
	}else{
		return -1;
	}
