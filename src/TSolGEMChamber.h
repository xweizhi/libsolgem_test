...
class TSolGEMChamber : public THaDetector {
 public:
  ...
  TSolGEMChamber() : fPlanes(0), fNPlanes(0), fShape(0) {} // for ROOT RTTI
  ...
  Int_t GetChamberType() const { return fChamberType; }  
  TGEMShape* GetShape() const {return fShape;};
  Double_t GetAngle() const {return fShape->GetAngle();}; // rotation angle between lab and wedge frame

  // Frame conversions
  void LabToPlane (Double_t& x, Double_t& y) const {fShape->LabToWedge (x, y);};  // input and output in meters
  void PlaneToLab (Double_t& x, Double_t& y) const {fShape->WedgeToLab (x, y);};  // input and output in meters
 ...
 private:
  Int_t fChamberType; //0 for wedge, 1 for rectangle
  TSolGEMPlane** fPlanes; // List of chambers
  UInt_t fNPlanes;
  TGEMShape* fShape;  // Wedge geometry
